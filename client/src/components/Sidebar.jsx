...
const Sidebar = () => {
  ...
  const {getUsers, users, selectedUser, setSeletedUser} = useContext(ChatContext)
    ...
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user,index)=>(
          ...
          key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm${selectedUser?._id === user._id && 'bg-[#282142z]/50'}`}>
